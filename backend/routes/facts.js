  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      const r = await axios.post(
        ...
        {
          contents: [{ parts: [{ text: 'Give me one surprising, little-known tech industry fact in exactly 2 sentences. Focus on something a CS student would find fascinating. No bullet points, no intro text.' }] }],
          generationConfig: { temperature: 0.9 },
        },
        { headers: { 'Content-Type': 'application/json' } }
      )
      const fact = r.data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim()
      if (fact) return res.json({ fact, source: 'ai' })
    } catch (e) {
      console.error('Facts AI error:', e.response?.data || e.message)
    }
  }
